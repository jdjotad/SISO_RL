...
class EnvPMSMTC(gym.Env):
    ...
    Torque Control in d-q reference frame.
    
    This environment simulates a PMSM with:
    - Direct-quadrature (d-q) rotor-oriented reference frame
    - Torque control capabilities
    - Accurate motor dynamics based on physical parameters
    
    The RL agent provides d-q voltage commands and learns to control the motor torque
    while minimizing current usage and voltage variations.
    """
    
    def __init__(self, sys_params, w_idq=0.1, w_vdq=0.1, render_mode=None):
        """
        Initialize the d-q frame PMSM Torque Control environment.
        ...
        self._init_system_parameters(sys_params, w_idq=w_idq, w_vdq=w_vdq)
    ...
    def _init_system_parameters(self, sys_params, w_idq, w_vdq):
        ...
        # Weighting factors for reward function
        self.w_idq = w_idq  # Weight for current magnitude
        self.w_vdq = w_vdq  # Weight for voltage variations
    ...
    def _init_action_space(self):
        """Initialize the action space for d-q voltages."""
        # Define normalized voltage limits [-1, 1]
        self.min_vd, self.max_vd = [-1.0, 1.0]
        self.min_vq, self.max_vq = [-1.0, 1.0]
    ...
    def _init_observation_space(self):
        ...
        # Observation bounds arrays: [te, te_ref, id, iq, we, prev_vd, prev_vq]
        self.low_observations = np.array(
            [self.min_te, self.min_ref_te, 
             self.min_id, self.min_iq, 
             self.min_we,
             self.min_vd, self.min_vq], 
            dtype=np.float32
        )
        self.high_observations = np.array(
            [self.max_te, self.max_ref_te, 
             self.max_id, self.max_iq, 
             self.max_we,
             self.max_vd, self.max_vq], 
            dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        Args:
            action (numpy.ndarray): d-q voltage commands [vd, vq]
            
        Returns:
            tuple: (observation, reward, terminated, truncated, info)
        """
        # Process action and update system state
        vd_t, vq_t = self._process_action(action)
        self._update_motor_state(vd_t, vq_t)
        
        # Generate observation
        obs, info = self._get_observation_and_info()
        
        # Calculate reward based on current state and torque reference
    ...
    def _process_action(self, action):
        ...
        Args:
            action (numpy.ndarray): d-q voltage commands normalized to [-1, 1]
            
        Returns:
            tuple: (vd, vq) - The processed voltage commands
        """
        ...
        norm_vdq = np.sqrt(np.sum(np.power(action_vdq, 2)))
        factor_vdq = self.vdq_max / norm_vdq if norm_vdq > self.vdq_max else 1
        
        # Apply limiting factor
        vd, vq = factor_vdq * action_vdq
        
        # Store these values for next step
        self.prev_vd, self.prev_vq = vd, vq
        
        return vd, vq
        
    def _update_motor_state(self, vd, vq):
        """
        Update motor state based on applied voltages.
        
        Args:
            vd (float): d-axis voltage
            vq (float): q-axis voltage
        """
    ...
    def _calculate_reward(self, te_norm, te_ref_norm, action):
        """
        Calculate the reward based on torque error, current magnitude, and voltage variations.
        
        Args:
            te_norm (float): Normalized torque
            te_ref_norm (float): Normalized reference torque
            action (numpy.ndarray): Current action [vd, vq]
        ...
        if self.reward_function == "absolute":
            return -(e_te + self.w_idq * i_dq_mag + self.w_vdq * (delta_vd + delta_vq))
            
        elif self.reward_function == "quadratic":
            return -(np.power(e_te, 2) + self.w_idq * np.power(i_dq_mag, 2) + 
                    self.w_vdq * (np.power(delta_vd, 2) + np.power(delta_vq, 2)))
                    
        elif self.reward_function == "quadratic_2":
            return -(np.power(e_te, 2) + self.w_idq * np.power(i_dq_mag, 2) + 
                    self.w_vdq * np.power(delta_vd + delta_vq, 2))
                    
        elif self.reward_function == "square_root":
            return -(np.power(e_te, 1/2) + self.w_idq * np.power(i_dq_mag, 1/2) + 
                    self.w_vdq * (np.power(delta_vd, 1/2) + np.power(delta_vq, 1/2)))
                    
        elif self.reward_function == "square_root_2":
            return -(np.power(e_te, 1/2) + self.w_idq * np.power(i_dq_mag, 1/2) + 
                    self.w_vdq * np.power(delta_vd + delta_vq, 1/2))
                    
        elif self.reward_function == "quartic_root":
            return -(np.power(e_te, 1/4) + self.w_idq * np.power(i_dq_mag, 1/4) + 
                    self.w_vdq * (np.power(delta_vd, 1/4) + np.power(delta_vq, 1/4)))
                    
        elif self.reward_function == "quartic_root_2":
            return -(np.power(e_te, 1/4) + self.w_idq * np.power(i_dq_mag, 1/4) + 
                    self.w_vdq * np.power(delta_vd + delta_vq, 1/4))
    ...
    def _initialize_motor_variables(self, options):
        ...
        if options:
            ...
            if all(k in options for k in ["prev_vd_norm", "prev_vq_norm"]):
                prev_vd_norm = np.float32(options["prev_vd_norm"])
                prev_vq_norm = np.float32(options["prev_vq_norm"])
    ...
    def _need_warmup_steps(self):
        ...
        return self.prev_vd is None or self.prev_vq is None
    ...
    def mtpa(self):
        ...
        """
        # For PMSM with saliency (Ld != Lq), MTPA requires optimization
        ...
        torque_constraint = {
            'type': 'eq', 
            'fun': lambda idq: self.te_ref - self.te_calculation(idq[0], idq[1])
        }
...
class EnvPMSMTCDataBased(gym.Env):
    """
    Reinforcement Learning environment for Permanent Magnet Synchronous Motor (PMSM)
    Torque Control in d-q reference frame using data-based model.
    ...
    def __init__(self, sys_params, w_idq=0.1, w_vdq=0.1, render_mode=None):
        """
        Initialize the d-q frame PMSM Torque Control environment using data-based model.
    ...
    def _init_transformation_tools(self):
        ...
        self.te_calculation = lambda psi_d, psi_q, id, iq: 3/2*self.p*(psi_d*iq - psi_q*id)
    ...
    def step(self, action):
        ...
        ad, bdw = self._setup_state_space_model(self.id, self.iq)
        self.ad = ad
        self.bd = bdw[:,:2]
    ...
    def _get_observation(self):
        ...
        """
        # Calculate torque
        psid = self.psid.interp2d(self.id, self.iq)
        psiq = self.psiq.interp2d(self.id, self.iq)
        te = self.te_calculation(psid, psiq, self.id, self.iq)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Setup state-space model
        self._setup_state_space_model(self.id, self.iq)
        
        # Initialize previous voltages or do warmup steps
        if self._need_warmup_steps():
            return self._perform_warmup_steps()
        else:
            return self._get_observation_and_info()
    ...
    def _perform_warmup_steps(self):
        ...
        for _ in range(n_init_steps):
            obs, _, _, _, info = self.step(action=self.action_space.sample())
    ...
    def _setup_state_space_model(self, id, iq):
        """
        Calculate the discrete state-space model matrices for the current operating point.
        
        Args:
            id (float): d-axis current
            iq (float): q-axis current
            
        Returns:
            tuple: (ad, bdw) - Discrete state-space matrices
        """
...
class EnvPMSMTCABC(gym.Env):
    ...
    def _get_observation_and_info(self):
        """Get observation and info dictionaries without warmup steps."""
